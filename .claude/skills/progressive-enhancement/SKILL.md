...
Modern CSS supports animations triggered by scroll position, without JavaScript.
...
| `exit 0%` | Element starting to leave |
| `exit 100%` | Element fully exited |
| `contain 0%` | Element fully contained (start) |
| `contain 100%` | Element fully contained (end) |
...
/* Scale while element is fully visible */
.scale-while-visible {
...
Subtle parallax without JavaScript:
...
Add shadow when header becomes sticky:
...
/* Stagger is automatic - each card animates when IT enters */
```

### Browser Support

Scroll-driven animations are supported in Chrome 115+, Edge 115+. For unsupported browsers, elements display without animation (progressive enhancement).
