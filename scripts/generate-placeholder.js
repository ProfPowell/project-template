 ...
 */
function generateSimple(width, height) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Placeholder image">
  <title>Placeholder image</title>
  <rect width="${width}" height="${height}" fill="${COLORS.background}"/>
  <line x1="0" y1="0" x2="${width}" y2="${height}" stroke="${COLORS.stroke}" stroke-width="1"/>
  <line x1="${width}" y1="0" x2="0" y2="${height}" stroke="${COLORS.stroke}" stroke-width="1"/>
</svg>`;
}
 ...
 * @returns {string} SVG content
 */
function generateLabeled(width, height, label) {
  ...
  <line x1="0" y1="0" x2="${width}" y2="${height}" stroke="${COLORS.stroke}" stroke-width="1"/>
  <line x1="${width}" y1="0" x2="0" y2="${height}" stroke="${COLORS.stroke}" stroke-width="1"/>
  <text x="${centerX}" y="${centerY}" text-anchor="middle" dominant-baseline="middle" font-family="system-ui, -apple-system, sans-serif" font-size="${fontSize}" fill="${COLORS.text}">${escapeXml(label)}</text>
</svg>`;
}
 ...
 * @returns {string} SVG content
 */
function generateBrand(width, height, label) {
  ...
  <line x1="0" y1="0" x2="${width}" y2="${height}" stroke="${COLORS.stroke}" stroke-width="1"/>
  <line x1="${width}" y1="0" x2="0" y2="${height}" stroke="${COLORS.stroke}" stroke-width="1"/>
  <rect x="${btnX}" y="${btnY}" width="${btnWidth}" height="${btnHeight}" rx="4" fill="${COLORS.accent}"/>
  <text x="${centerX}" y="${centerY}" text-anchor="middle" dominant-baseline="middle" font-family="system-ui, -apple-system, sans-serif" font-size="${fontSize}" fill="${COLORS.accentText}">${escapeXml(label)}</text>
</svg>`;
}
...
function main() {
  ...
  switch (args.type) {
    case 'simple':
      svg = generateSimple(args.width, args.height);
      break;
    case 'labeled':
      svg = generateLabeled(args.width, args.height, args.label);
      break;
    case 'brand':
      svg = generateBrand(args.width, args.height, args.label);
      break;
  }
